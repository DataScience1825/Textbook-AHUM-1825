...
```{r prepare-hmw-_starter_-1, echo=FALSE}
...
#### Problem 1
```{r _starter__hmw1, echo=FALSE, exercise.startover = TRUE}
...
```{r _starter__hmw2, exercise = TRUE, exercise.blanks = "___+", exercise.completion = FALSE, exercise.setup = "prepare-hmw-_starter_-1"}
complete code ____ by filling ____ in blanks _____ 
```

```{r _starter__hmw2-check}
grade_result(
...
```{r _starter__hmw3, exercise = TRUE, exercise.completion = FALSE, exercise.setup = "prepare-hmw-_starter_-1"}

```  

```{r _starter__hmw3-check}
grade_result(
  pass_if(~identical(.result, TRUE))#Replace TRUE with the solution's code
)
```
