...
<!-- Each Learnr Coding Exercise is run in an isolated environment. -->
<!-- A "prepare" chunk is needed for each question to access the -->
<!--   homework dataset, which should be stored in the local data-->
<!--   directory as hmw_data.csv -->
```{r prepare-hmw-filter_lab-1, echo=FALSE}
...
#### Problem 1
```{r filter_lab_hmw1, echo=FALSE, exercise.startover = TRUE}
...
```{r filter_lab_hmw2, exercise = TRUE, exercise.blanks = "___+", exercise.completion = FALSE, exercise.setup = "prepare-hmw-filter_lab-1"}
complete code ____ by filling ____ in blanks _____ 
```

```{r filter_lab_hmw2-check}
grade_result(
...
```{r filter_lab_hmw3, exercise = TRUE, exercise.completion = FALSE, exercise.setup = "prepare-hmw-filter_lab-1"}

```  

```{r filter_lab_hmw3-check}
grade_result(
  pass_if(~identical(.result, TRUE))#Replace TRUE with the solution's code
)
```
