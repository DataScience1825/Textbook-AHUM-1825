...
#These two attributes can be updated with the updateAllHomeworks() or updateOneHomework()
#hmw_dataset <- "FarmInventory"#&& <-don't remove "#&&"
#hmw_number <- 1#&& <- don't remove "#&&"

#hmw_data_desc <- paste("../SharedResources/datasets/",hmw_dataset,"/description.Rmd", sep="")
#hmw_file <- paste("homework_",hmw_number,".Rmd", sep="")

##Extra practice problems
farm_data <- read_csv(here::here("SharedResources","datasets","FarmInventory","data.csv"), col_types="ccddffi");
...
```{r, child=here::here("SharedResources","datasets","PalmerPenguins","description.Rmd")}
...
expression_S <- reactive(
  switch(input$compareOp_S, 
    ...
    "Not Equal to (!=)"=paste("result <- filter(penguins, var != <span style=\"color:red\">", input$slider_S, "</span>)\nresult", sep=""),
...
```{r include_hmw_grader, child=here::here("SharedResources","grading","hmw_grader.Rmd")}
```
```{r include_hmw_dwnld, child=here::here("SharedResources","grading","download_homework.Rmd")}
```
...
```{r  child=here::here("SharedResources","datasets","FarmInventory","description.Rmd")}
```
...
grade_result(
  pass_if(~identical(.result, filter(farm_data, p_name == "Russian Blue Potato" | p_name == "Purple Sweet Potato", p_per_lb * q_in_lbs > 3000)))
)
```
...
grade_result(
  pass_if(~identical(.result, filter(farm_data, str_detect(p_name, "Beef"), type=="MEAT")))
)
```
...
grade_result(
  pass_if(~identical(.result, filter(farm_data, is.na(avg_sl))))
)
```
...
grade_result(
  pass_if(~identical(.result, filter(farm_data, p_name %in% groceries, f_name == "Sevier")))
)
```
